...
def get_predicted_volumes(pred):
    ...
    Arguments:
        pred {Numpy array} -- array with labels. Assuming 0 is bg, 1 is anterior, 2 is posterior
                              # notes: pred.shape = (d0, d1, d2)

    Returns:
        A dictionary with respective volumes
    """
    volume_ant = np.count_nonzero(pred.astype(int)==1)
    volume_post = np.count_nonzero(pred.astype(int)==2)
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    slice_indeces = [j*n_slices//9 for j in range(9)]
    slice_indeces[-1] -= 1

    # pad orig val with zeros
    new_orig_vol = np.zeros(pred_vol.shape)
    new_orig_vol[:orig_vol.shape[0], :orig_vol.shape[1], :orig_vol.shape[2]] = orig_vol
    orig_vol = np.array(new_orig_vol)
    ...
    for i, slice_i in enumerate(slice_indeces):
        # box to plot the figure
        fig_row = i//3
        fig_col = i%3
        fig_box_x = 10 + (gap+fs)*fig_col
        fig_box_y = 150 + (fs)*fig_row
        fig_box = (fig_box_x, fig_box_y)

        # transform the original image
        orig_img_np = np.flip((orig_vol[slice_i, :, :]/np.max(orig_vol[slice_i, :, :]))*0xff).T.astype(np.uint8)  # (patch_size, patch_size)
        # transform the predicted mask
        pred_img_np = np.flip(pred_vol[slice_i, :, :]).T.astype(np.uint8)  # (patch_size, patch_size)
        # predicted image
        posterior_np = np.zeros(pred_img_np.shape)
        anterior_np = np.zeros(pred_img_np.shape)
        # dstack to RGB
        posterior_np = np.dstack((posterior_np, posterior_np, posterior_np))
        anterior_np = np.dstack((anterior_np, anterior_np, anterior_np))
        # anterior hippocampus (label = 1) --> red
        anterior_np[pred_img_np==1] = [1, 0, 0]
        anterior_mask = Image.fromarray(np.uint8(255*anterior_np[:, :, 0])).resize(fig_size)
        # posterior hippocampus (label = 2) --> blue
        posterior_np[pred_img_np==2] = [0, 0, 1]
        posterior_mask = Image.fromarray(np.uint8(255*posterior_np[:, :, 2])).resize(fig_size)
        # paste to panel
        pimg.paste(Image.fromarray(orig_img_np, mode="L").convert("RGBA").resize(fig_size), box=fig_box)
        pimg.paste(Image.fromarray(np.uint8(255*anterior_np), mode="RGB").resize(fig_size), box=fig_box, mask=anterior_mask)
        pimg.paste(Image.fromarray(np.uint8(255*posterior_np), mode="RGB").resize(fig_size), box=fig_box, mask=posterior_mask)
        # figure title
        draw.text((fig_box_x-30+fs//2, fig_box_y+100), "Slice #{}/{}".format(slice_i, n_slices), (255, 255, 255), font=main_font)

    # add report headings/main texts
    draw.text((10, 0), "HippoVolume.AI", (255, 255, 255), font=header_font)

    report_texts = [f"Patient ID: {header.PatientID}\n",
                    f"Hippocampal volumes (in pixel^3):\n",
                    f"    Anterior = {inference['anterior']} (shown in red)\n",
                    f"    Posterior = {inference['posterior']} (shown in blue)\n",
                    f"    Total = {inference['total']}"]

    draw.multiline_text((10, 90), "".join(report_texts), fill=(255, 255, 255), font=main_font)
...
def get_series_for_inference(path):
    ...
    for x in dicoms:
        if x.SeriesDescription == "HippoCrop":
            series_for_inference.append(x)
